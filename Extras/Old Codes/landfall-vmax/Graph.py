...
# Extract and plot for Eastern countries
for country in East:
...
bins = np.arange(min(East_landfall), max(East_landfall) + binwidth, binwidth)

# Plot a histogram of landfall vs latitude
plt.figure()
plt.title('Eastern Number of Landfalls vs Latitude')
plt.hist(East_landfall, bins=bins)

# Put bins index for latitude
inds = np.digitize(East_landfall,bins)
...
while bin_number < len(bins):
	...
	while latitude_number < len(East_landfall):
		# Check for points in the bins
		if inds[latitude_number] == bin_number:
			# If the point is in the bin, add vmax to list vmaxs
			vmaxs.append(East_vmax[latitude_number])
		latitude_number += 1
