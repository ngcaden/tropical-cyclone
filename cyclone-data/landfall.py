...
# METHOD
# Define the relative path of the current working folder
REL_PATH = os.path.dirname(os.path.abspath(__file__))


print os.path.dirname(os.path.realpath(__import__("__main__").__file__))

os.chdir('/Users/nguyenquang30795/Desktop/BSc Project/cyclone-data')

# Read the data file
with open('cyclone-track-full', 'rb') as file:
    source = file.read()
    cyclone_track = json.loads(source)

# Read the latlon file
with open('taiwan-latlon3', 'rb') as file:
    source = file.read()
...
while cyclone_number < len(cyclone_track):
    ...
    while point_number < (len(each_cyclone_track[0])-1):
        longitude = (each_cyclone_track[0])[point_number]
        latitude = (each_cyclone_track[1])[point_number]
        
        next_longitude = (each_cyclone_track[0])[point_number+1]
        next_latitude = (each_cyclone_track[1])[point_number+1]
...
with open('taiwan-landfall-location','wb') as dump:
